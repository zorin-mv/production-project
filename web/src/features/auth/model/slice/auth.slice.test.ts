...
describe('auth-slice-test', () => {
  test('test set email', () => {
    ...
    expect(
      authReducer(
        state as IAuthSchema,
        authActions.setEmail('changedtest@mail.com')
      )
    ).toEqual({ email: 'changedtest@mail.com' });
  });
  ...
  test('test set password', () => {
    ...
    expect(
      authReducer(
        state as IAuthSchema,
        authActions.setPassword('changedpassword')
      )
    ).toEqual({ password: 'changedpassword' });
  });
});
